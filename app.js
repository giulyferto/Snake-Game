document.addEventListener('DOMContentLoaded', () => {
    ...
    const width = 10
    let currentIndex = 0 // current snake location
    let appleIndex = 0 // location of apple
    ...
    function moveOutcomes() {
        ...
        currentSnake.unshift(currentSnake[0] + direction) // adds the new head to the beginning of the array
    ...
    function control(e) {
        squares[currentIndex].classList.remove('snake') // remove snake class from current square
        if (e.keyCode === 39) {
            direction = 1
        } else if (e.keyCode === 38) {
            direction = -width // here the snake will go up 10 divs appearing to go up
        } else if (e.keyCode === 37) {
            direction = -1
        } else if (e.keyCode === 40) {
            direction = +width // here the snake will go down 10 divs appearing to go down
        }
    }
